...
pub enum MortonIndexNaming {
    ...
    /// `x-index: 15`, `y-index: 7`, `z-index: 3`. Each index is a number in `[0;(2^N)-1]`.
    GridIndex,
}
...
pub trait MortonIndex : PartialOrd + Ord + PartialEq + Eq + Debug + Hash {
    ...
    fn get_cell_at_level(&self, level: usize) -> <<Self as MortonIndex>::Dimension as crate::dimensions::Dimension>::Cell;
    ...
    unsafe fn get_cell_at_level_unchecked(&self, level: usize) -> <<Self as MortonIndex>::Dimension as crate::dimensions::Dimension>::Cell;
    ...
    /// 
    /// If `level` is `>= self.depth()`
    fn set_cell_at_level(&mut self, level: usize, cell: <<Self as MortonIndex>::Dimension as crate::dimensions::Dimension>::Cell);
    /// Sets the cell at the given `level` to `cell`. Performs no depth checks.
    /// 
    /// # Safety
    /// 
    /// Calling this function with a value for `level` that is `>= self.depth()` is UB
    unsafe fn set_cell_at_level_unchecked(&mut self, level: usize, cell: <<Self as MortonIndex>::Dimension as crate::dimensions::Dimension>::Cell);
    ...
    /// Converts this `MortonIndex` into a N-dimensional grid index
    fn to_grid_index(&self) -> <Self::Dimension as crate::dimensions::Dimension>::GridIndex;
}