...
pub trait MortonIndex : PartialOrd + Ord + PartialEq + Eq + Debug + Hash {
    ...
    fn get_cell_at_level(&self, level: usize) -> <Self::Dimension as crate::dimensions::Dimension>::Cell;
    ...
    unsafe fn get_cell_at_level_unchecked(&self, level: usize) -> <Self::Dimension as crate::dimensions::Dimension>::Cell;
    ...
    /// 
    /// If `level` is `>= self.depth()`
    fn set_cell_at_level(&mut self, level: usize, cell: <Self::Dimension as crate::dimensions::Dimension>::Cell);
    /// Sets the cell at the given `level` to `cell`. Performs no depth checks.
    /// 
    /// # Safety
    /// 
    /// Calling this function with a value for `level` that is `>= self.depth()` is UB
    unsafe fn set_cell_at_level_unchecked(&mut self, level: usize, cell: <Self::Dimension as crate::dimensions::Dimension>::Cell);
    ...
    /// Converts this `MortonIndex` into a N-dimensional grid index, using the given `ordering`
    fn to_grid_index(&self, ordering: <Self::Dimension as crate::dimensions::Dimension>::CellOrdering) -> <Self::Dimension as crate::dimensions::Dimension>::GridIndex;
}